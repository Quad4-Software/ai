...
// Command rns is a stdio MCP server exposing the Reticulum
...
import (
	...
	"github.com/Quad4-Software/ai/mcp/rns/internal/community"
	"github.com/Quad4-Software/ai/mcp/rns/internal/docs"
	"github.com/Quad4-Software/ai/mcp/rns/internal/mcp"
	"github.com/Quad4-Software/ai/mcp/rns/internal/sysutil"
)
...
- Pages are written in Micron markup; the micron server can parse, lint and render Micron.
	...
	return []mcp.Tool{
		...
		},
		{
			...
			InputSchema: obj(map[string]any{
				...
				"rns_url":    strArg("RNS URL, e.g. rns://06a54b505bb67b25ef3f8097e8001edc/public/LXMFy"),
				"args":       arrayArg(strArg("Additional flags or targets"), "Optional extra flags like --wheel, --public, or a simple target"),
			}, "subcommand"),
			Handle: func(ctx context.Context, args json.RawMessage) (string, error) {
				var a struct {
					Subcommand string   `json:"subcommand"`
					RNSURL     string   `json:"rns_url"`
					Args       []string `json:"args"`
				}
				if err := json.Unmarshal(args, &a); err != nil || a.Subcommand == "" {
					return "", fmt.Errorf("missing required argument: subcommand")
				}
				return util.Rngit(ctx, a.Subcommand, strings.TrimSpace(a.RNSURL), a.Args)
			},
		},
		{
			Name:        "unsigned_posts",
...
func main() {
	...
	srv := mcp.NewServer("rns", "0.3.0", tools(src, sysutil.New()), prompts(src))
	if err := srv.Serve(context.Background(), os.Stdin, os.Stdout); err != nil {
		fmt.Fprintln(os.Stderr, "rns:", err)
		os.Exit(1)
	}
}
