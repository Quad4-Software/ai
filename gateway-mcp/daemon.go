...
// Shared-daemon transport for gateway-mcp: one long-lived process owns
...
func defaultSocket() string {
	if d := os.Getenv("XDG_RUNTIME_DIR"); d != "" {
		return filepath.Join(d, "gateway-mcp.sock")
	}
	return filepath.Join(os.TempDir(), fmt.Sprintf("gateway-mcp-%d.sock", os.Getuid()))
}
...
func runDaemon(ctx context.Context, sock string, srv *mcp.Server) error {
	...
	defer stop()
	fmt.Fprintf(os.Stderr, "gateway-mcp: daemon listening on %s\n", sock)
