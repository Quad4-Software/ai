...
- `#nosec` requires an inline justification comment.
- Offline builds/tests: no network fetches in build or test paths.
...
nodwarf5 build), so `make golangci` may not work locally; rely on
`make vet` + `make gosec` + CI instead. Do not switch go.mod toolchain
lines to fix it.

## Vendored parser (micron-mcp)
...
See `.devin/skills/` for detailed guides: `mcp-toolkit`, `reticulum-mesh`,
