...
`.github/workflows/` contains: `ci.yml`, `dependency-review.yml`,
`race.yml`, `fuzz.yml`, `bench.yml`, `leak.yml`. There is no release
workflow yet (see the release skill for the intended process).
...
- Every job starts with `step-security/harden-runner` at a pinned SHA
  ...
  egress audit/blocking as appropriate.
...
- Dependabot (`.github/dependabot.yml`) manages `gomod` weekly, limit 5 PRs.
  Keep it covering the root and consider per-module entries for `*-mcp` dirs.
