...
Micron is NomadNet's lightweight markup:

- Headings: `>`, `>>`, `>>>` for decreasing depth.
- Bold/italic/underline: `*bold*`, `!italic!`, `_underline_` (star, bang,
  underscore delimiters).
- Colors: backtick-quoted color directives and named color blocks.
- Alignment: centered/right-aligned via heading modifiers.
- Links: `` `[url|label]` `` style link fields; schemes validated.
- Forms: input fields and submit actions rendered as form elements.
- Tables, partials (with data descriptors), and literal/preformatted blocks.
...
- `extract_links` - URL, label, source line, field names.
...
- HTML output is escaped; `javascript:`, `vbscript:`, `data:`, and `file:`
  link schemes are rejected/neutralized in the parser (`url.go`, `lint.go`,
  `html_build.go`). Do not weaken these checks.
- No raw HTML passthrough: Micron never emits `<script>`, event-handler
