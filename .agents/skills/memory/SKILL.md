...
name: memory
description: >
  This skill persists short-term agent memory through memory-mcp.
...
1. Build memory-mcp: `cd memory-mcp && go test ./... && go build`.
...
- "Remember that `rns-mcp` stores config in `~/.reticulum` with tag `config`."
